...
public class ProcessServiceManager
{
    ...
    public static ServiceStatus GetStatus(DevServiceInfo service)
    {
        ...
        // Fallback: check if port is currently in use by another process
        if (service.Port > 0)
        {
    ...
    public static bool StopProcess(DevServiceInfo service)
    {
        try
        {
            ...
            // Fallback: kill by PID if identified via port
            if (service.Port > 0)
            {
                int? pid = PortConflictDetector.GetProcessIdByPort(service.Port);
                if (pid.HasValue)
                {
                    var externalProc = Process.GetProcessById(pid.Value);
                    externalProc.Kill(true);
                    service.ProcessId = null;
                    service.Status = ServiceStatus.Stopped;
                    AppLogger.Log($"Terminated {service.Name} PID {pid.Value} bound to port {service.Port}");
                    return true;
                }
            }
        }
